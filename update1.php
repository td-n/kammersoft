...
$sql = mysql_query("SELECT * FROM `conf` WHERE wert = 'instverz' ");         //Tabelle conf auswählen
while($ds= mysql_fetch_object($sql))
{
    $instverz = $ds->was;
}
if (isset($instverz))
{
    echo "Update3 bereits eingespielt";
}
else
{
    $query = "INSERT INTO `conf` (`wert`)VALUES ('instverz')";
    mysql_query($query);
    echo "Update3 eingespielt";
}
